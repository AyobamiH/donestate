...
describe("hosted Codex executor contract", () => {
  ...
  it("launches Codex exactly once with the Sandbox background-process primitive and reconciles only by terminal receipt", () => {
    ...
    expect(IMPLEMENTATION_BACKGROUND_PROCESS_COMMAND).toBe('setsid -f -w /bin/sh "$DONESTATE_RECEIPT_SCRIPT_PATH" > "$DONESTATE_RECEIPT_LOG_PATH" 2>&1');
