...
The Worker provides GitHub OAuth for public repositories, an encrypted per-user OpenAI credential vault, one Durable Object per run, isolated Cloudflare Sandbox execution, exact-head branch and pull-request publication, durable effect reconciliation, deletion, and signed independent-verifier handoff. Model usage is billed to each user's own OpenAI API account, never a shared DoneState key. The hosted service is deployed at [donestate.proofandstate.com](https://donestate.proofandstate.com); its public-repository execution has a historical verified canary. The `workers.dev` hostname remains enabled only as a temporary transport compatibility surface for the already-submitted OpenAI review version. GitHub App maintenance is active for the single selected repository, and directory version 0.3.0 is in OpenAI Review (8 September 2026); the historical 0.2.0 disposition is unknown. Separately, the GitHub Marketplace submission remains `SUBMITTED / IN_REVIEW` and `Pending for publish`. On 1 September 2026, GitHub's authenticated edit page said the listing was a draft and had not been published. The owner [preview](https://github.com/marketplace/donestate) displayed provider `AyobamiH`, a `$0` `Public repositories` plan, `Add`, `Install it for free`, and `1 install`, but an unauthenticated exact Marketplace search returned no result. The preview and authenticated listing inventory do not prove public availability, webhook delivery, entitlement, OAuth completion, repository selection, execution, billing, retention, or a user outcome. The [generated project state](docs/PROJECT-STATE.md) is the canonical work ledger and Evidence Story Bank; CI rejects consequential changes that do not update it. See [Current status](docs/CURRENT-STATUS.md), [GitHub Marketplace listing](docs/GITHUB-MARKETPLACE.md), [owned-domain cutover evidence](docs/OWNED-DOMAIN-CUTOVER.md), [Hosted plugin](docs/HOSTED-PLUGIN.md), and [OpenAI review submission](docs/OPENAI-REVIEW.md).
