...
Maintenance branch retirement is deliberately narrower than maintenance publication. DoneState may retire only its own exact `donestate/<run-id>` branch after the corresponding maintenance run is independently `VERIFIED`, the exact pull request is closed as merged, and repository, base ref, branch, head SHA and pull-request number all still match the sealed run. Closed-unmerged, failed, uncertain, historical and foreign branches are preserved. The signed pull-request webhook attempts retirement immediately, while the hourly sweep reconciles verified merged branches if event ordering or an already-deleted ref prevented the immediate path. Branch-retirement failure never changes the verifier decision or grants merge authority.
